// Generated on 2016-07-04 using generator-ionic 0.8.0
'use strict';
...
module.exports = function (grunt) {
  ...
  grunt.initConfig({
    ...
    ngconstant: {
      ...
      staging: {
        constants: {
          ...
          Environment: {
            name: 'staging',
            api: `http://${grunt.option('ngrok')}.ngrok.io/`,
          }
        }
      },
      production: {
        constants: {
          ...
          Environment: {
            name: 'production',
            api: 'https://moneyisagame.herokuapp.com/',
          }
        }
      }
    },

    // Replace cordova with mocks
    replace: {
      index: {
        src: [
          'www/index.html',
      ...
      app: {
        ...
        replacements:[
          ...
          // {
          //   from: "// AutoLogin HERE",
          //   to: "Auth.login({username:'rep',password:'test'}).then(function(){console.log('AUTO LOGIN')})"
          }
        ]
      }
    },
    ...
    watch: {
      bower: {
        files: ['bower.json'],
        tasks: ['babel', 'wiredep', 'newer:copy:app']
      },
      html: {
        files: ['<%= yeoman.app %>/**/*.html'],
        tasks: ['newer:copy:app','replace:index']
      },
      js: {
        files: ['<%= yeoman.app %>/<%= yeoman.scripts %>/{**/,}**/*.js'],
        tasks: ['injector','babel','newer:copy:app','newer:copy:tmp', 'replace:app', 'newer:jshint:all']
      },
      ngdocs: {
        files: ['<%= yeoman.app %>/<%= yeoman.scripts %>/**/*.js'],
        tasks: ['ngdocs']
      ...
      compass: {
        files: [
          '<%= yeoman.app %>/<%= yeoman.scripts %>/**/*.{scss,sass}',
    ...
    //   app: {
    //     files: {
    //       '<%= yeoman.app %>/styles/main.scss' : '<% yeoman.app %>/styles/**/*.scss'
    //     }
    //   }
    // },
    ...
    babel: {
      ...
      dist: {
        files: [{
          ...
          src: ['**/**/*.js','**/*.js','*.js'],
          dest: '.temp/scripts/',   // Destination path prefix.
    ...
    // ngdocs creates application documentation
    ngdocs: {
      options: {
        dest: 'docs'
      },
  ...
  grunt.registerTask('serve', function (target) {
    ...
    return grunt.task.run(['injector','wiredep', 'init:' + target, 'replace', 'concurrent:ionic']);
  });

  grunt.registerTask('emulate', function(target) {
    target = target || 'development';
    grunt.config('concurrent.ionic.tasks', ['ionic:emulate:' + this.args.join(), 'watch']);
    return grunt.task.run(['init:' + target, 'concurrent:ionic']);
  });

  grunt.registerTask('run', function(platform)  {
    platform = platform || 'ios';
    return grunt.task.run(['init:staging', 'ionic:run:' + platform]);
  });

  grunt.registerTask('build', function(target,platform) {
    target = target || 'production';
    platform = platform || 'ios';
    console.log(`Target: ${target}`);
    return grunt.task.run([`init:${target}`,'ionic:build:' + platform]);
  });
